...
export class ThesisImproverService {
  ...
  private async improveWithAi(
    ...
    const prompt = [
      'Ты синтезируешь идеи после раунда диалога где каждый участник внёс уникальный вклад.',
      'Это не дебат — это система коллективного генерирования идей.',
      'Каждый участник добавил новую перспективу, аналог или вопрос для исследования.',
      'Твоя задача — создать более богатый и конкретный синтез интегрируя лучшие идеи.',
      '',
      `ИСХОДНЫЙ ТЕЗИС: ${debate.originalThesis}`,
      `ТЕКУЩИЙ ТЕЗИС: ${debate.currentThesis}`,
      '',
      `ВКЛАДЫ УЧАСТНИКОВ В РАУНДЕ ${roundNumber}:`,
      attackLines,
      '',
      'Создай синтез. Ответ ОБЯЗАН содержать ровно три блока и новый тезис:',
      '',
      '[1] ИЗМЕНИЛОСЬ: какая новая идея или перспектива была интегрирована (одно конкретное предложение)',
      '[2] ИНТЕГРИРОВАНО: чей вклад вошёл в синтез и как именно (явная ссылка на роль)',
      '[3] ЕЩЁ ОТКРЫТО: какой вклад или вопрос ещё не нашёл места в тезисе (честно)',
      '',
      'УЛУЧШЕННЫЙ_ТЕЗИС: <синтез — богаче, конкретнее, с интегрированными идеями участников>',
      '',
      'Требования:',
      '- Тезис должен стать конкретнее и содержательнее, не длиннее',
      '- Должен явно интегрировать хотя бы одну новую идею из вкладов',
      '- Нельзя уклоняться: если участник предложил конкретный механизм — учти его',
      '- Цель — более богатая идея, а не защита от критики',
    ].join('\n');
  ...
  private parseAiResponse(
    ...
    const integrated = this.extractSection(raw, '[2] ИНТЕГРИРОВАНО') ??
      this.extractSection(raw, '[2] CLOSED') ?? '';
    const stillOpen = this.extractSection(raw, '[3] ЕЩЁ ОТКРЫТО') ??
      this.extractSection(raw, '[3] STILL_WEAK') ??
      this.extractSection(raw, '[3] STILL WEAK') ?? '';

    const summary = [changed, integrated, stillOpen]
      .filter(Boolean)
      .join(' | ')
